    ...
    ctx
      ...
      .action(({ meta, options }) => {
        if (options.encode && typeof options.encode !== 'boolean') {
          const encode = options.encode.toString().trim();
          if (encode.length < 1) {
          ...
          const code = options.decode.toString().trim();
          if (code.length < 1) {
            return meta.$send(buildMessage(meta, '没有内容可以解码'));
          }
          return meta.$send(buildMessage(meta, Buffer.from(code, 'base64').toString('utf-8')));
        } else {

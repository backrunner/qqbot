...
function requestAPI(meta, params) {
  return new Promise((resolve) => {
    axios
      .post(`${COMMON_API}/qrcode/decode`, params, { timeout: 5000 })
      .then((res) => {
        if (!res) {
          clearTimeout(
            timer_decode[`${meta.messageType === 'group' ? meta.groupId : 'user'},${meta.userId}`],
          );
          timer_decode[
            `${meta.messageType === 'group' ? meta.groupId : 'user'},${meta.userId}`
          ] = null;
...
module.exports = {
  ...
  apply: (ctx) => {
      ...
      .action(async ({ meta, options }) => {
        if (typeof options.encode === 'string') {
          return meta.$send(
            buildMessage(
              meta,
              `${COMMON_API}/qrcode/encode?text=${encodeURIComponent(options.encode)}`,
            ),
          );
        } else if (typeof options.decode === 'string') {
          return requestAPI(meta, { base64: options.decode });
        } else if (typeof options.url === 'string') {
          return requestAPI(meta, { url: options.url });
        } else {
    ...
    ctx.middleware(async (meta, next) => {
      ...
      // 信息抽出
      const message = meta.$parsed.message.replace(regex_encode, '').trim();
      if (message.length < 1) {
        return meta.$send(buildMessage(meta, '请输入有效的编码信息'));
      }
      return meta.$send(
        buildMessage(meta, `${COMMON_API}/qrcode/encode?text=${encodeURIComponent(message)}`),
      );
    });
    // 解码中间件
    ctx.middleware(async (meta, next) => {
      ...
      if (timer_decode[`${meta.groupId},${meta.userId}`]) {
        const image = extractImage(meta.$parsed.message);
        if (!image) {
          return next();
        }
        return requestAPI(meta, { url: image });
      } else {
        // 匹配到关键词才触发逻辑
